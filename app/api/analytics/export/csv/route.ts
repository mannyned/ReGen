...
const roleGuard = createRoleGuardMiddleware('analytics_export_csv')
  ...
  return roleGuard(request, async (req: NextRequest, user: UserContext) => {
    try {
      ...
      const options: ExportOptions = {
        format: 'csv',
        aggregation: (searchParams.get('aggregation') as 'per_post' | 'per_day' | 'aggregated') || 'per_post',
        filters,
        includeRetention: searchParams.get('includeRetention') === 'true',
        includeLocationData: searchParams.get('includeLocationData') === 'true',
      }
      ...
      return NextResponse.json({
        success: true,
        message: 'CSV export job created',
        jobId: result.jobId,
      ...
      return NextResponse.json(
        {
          success: false,
          error: error instanceof Error ? error.message : 'Failed to create CSV export',
        },
        { status: 500 }
      )
    }
...
// Create CSV export with body parameters (for more complex filters)
// ============================================
