...
function resolveBranding(whiteLabel?: WhiteLabelOptions): BrandingConfig {
  if (!whiteLabel?.enabled) {
    return {
      ...
      footerText: 'Generated by ReGenr • AI-Powered Social Media Analytics',
      hideReGenrBranding: false,
    }
  }

  return {
    companyName: whiteLabel.companyName || 'ReGenr',
    ...
    footerText: whiteLabel.footerText || `Generated by ${whiteLabel.companyName || 'ReGenr'}`,
    hideReGenrBranding: whiteLabel.hideReGenrBranding || false,
...
<body>
  ...
  private generateCoverPage(data: PDFReportData, branding: BrandingConfig): string {
    ...
    const logoSection = branding.companyLogo
      ...
      : `
        <div class="logo-section">
          <div class="logo">
            ${!branding.hideReGenrBranding ? '<span class="logo-icon">✨</span>' : ''}
            <span class="logo-text">${this.escapeHtml(branding.companyName)}</span>
          </div>
          <div class="tagline">${branding.hideReGenrBranding ? 'Social Media Analytics' : 'AI-Powered Social Media Analytics'}</div>
        </div>
      `
      ...
      <div class="cover-content">
        ...
        <div class="date-range">
          ...
          <div class="date-value">
            ${this.formatDate(data.dateRange.from)} — ${this.formatDate(data.dateRange.to)}
          </div>
        </div>
        ...
        <div class="generated-info">
          ...
          ${!branding.hideReGenrBranding ? '<br/><span class="powered-by">Powered by ReGenr</span>' : ''}
        </div>
      </div>
    ...
    const platformRows = data.platformAnalytics
      ...
      <tr>
        ...
        <td class="growth-cell ${platform.growth >= 0 ? 'positive' : 'negative'}">
          ${platform.growth >= 0 ? '↑' : '↓'} ${Math.abs(platform.growth).toFixed(1)}%
        </td>
      </tr>
    `
      )
  ...
  private generateFooterScript(data: PDFReportData, branding: BrandingConfig): string {
    const footerText = branding.hideReGenrBranding
      ? `${branding.footerText} • ${this.formatDateTime(data.generatedAt)}`
      : `${branding.footerText} • ${this.formatDateTime(data.generatedAt)}`
    ...
    <script>
      ...
      window.reportMetadata = {
        ...
        hideReGenrBranding: ${branding.hideReGenrBranding}
      };
    </script>
    `
  }
  ...
  private generateFileName(userId: string, data: PDFReportData, branding?: BrandingConfig): string {
    ...
    // Use company name for white-label exports, otherwise use ReGenr
    const prefix = branding?.hideReGenrBranding && branding.companyName !== 'ReGenr'
